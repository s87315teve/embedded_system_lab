...
class ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate
{
    ...
    var sortedDataArray = [[String:Any]]()
...
//        cell.detailTextLabel?.text = "\(cost)"
        ...
        let cost = record["cost"] as? Double ?? 0.0
...
//        TODO:更新總額到 totalCostLabel
        ...
        let groupedArray = Dictionary(grouping: dataArray, by: { (item: Dictionary<String, Any>) -> String in
            let currentDate=formatter.string(from: item["date"] as! Date)
            return currentDate
        })
        print("\(groupedArray)" )
        // for debug
...
//            print(stringArray)
    ...
    func groupDataArray() {
        ...
        // 使用Dictionary分组并创建Section数组
        let groupedDictionary = Dictionary(grouping: dataArray) { (item: Dictionary<String, Any>) -> Date in
            return item["date"] as? Date ?? Date()
        }
    }
